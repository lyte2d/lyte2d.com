...
local TagMeta = {
    ...
    __tostring = function(tag)
        if tag.options.is_singular then
            ...
            return string.format("<%s%s%s>%s", tag.name, tostring(tag.attributes), closing_slash, tag:content())
        else
            return string.format("<%s%s>%s</%s>", tag.name, tostring(tag.attributes), tag:content(), tag.name)
        end
    end
}
